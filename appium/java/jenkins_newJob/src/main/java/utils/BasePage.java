...
public class BasePage {
    ...
    */
    ...
    public void swipeUp() {
        ...
        this.doSleep ();
        (new TouchAction (SingleDriver.getInstance())).
                press(PointOption.point(width/2, height-200)).waitAction(WaitOptions.waitOptions(Duration.ofSeconds(1)))
                .moveTo(PointOption.point(width/2, 200)).release().perform();
    }
    ...
    public String getNativePageTitle(String strTitle) {
        By byTitle = By.id (strTitle);
        return this.getWebElementText (byTitle);
    }

    public String getWebPageTitle() {
        this.getWebDriver ();
        By byTitle = By.className ("hna-header-title");
        MobileElement waitTitle = (MobileElement) new WebDriverWait (SingleDriver.getInstance(), 25)
                .until (ExpectedConditions.visibilityOfElementLocated (byTitle));

        return find (byTitle).getText ();
    }

    public void goBackToLevelOnePage() {
        this.getWebDriver ();
        By byLeftHeader = By.className ("hna-header-left");
        MobileElement waitLeftHeader = (MobileElement) new WebDriverWait (SingleDriver.getInstance(),25)
                .until(ExpectedConditions.visibilityOfElementLocated(byLeftHeader));
        WebElement backIcon = find (byLeftHeader).findElement (By.className ("hna-header-btn"));
        backIcon.click ();
    }
}
